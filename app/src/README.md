...
npm run dev:full # Vite + local Node API server (Gemini + national scholarship DB)
npm test         # node built-in test runner
npm run build    # writes ../public/tool, which the site serves at /tool/
```

This is the source for the tool half of the site. The marketing pages and
Firebase login live in `../public`, and the build output goes inside that folder,
so the whole thing deploys as one static site. See `../INTEGRATION.md`.
...
- The profile lives in React state for the session. It is only written to
  Firestore for a signed-in user who turns on "Save my profile" on the account
  page, and turning that off deletes what was stored.
...
- **Auth:** Firebase, shared with the marketing site. "Log in" sends people to
  `/login.html`; the tool reads the session and shows an account tab. Nothing is
  gated behind it, signing in only adds saved profiles and the email digest.
...
4. Run `npm install`.
5. Run `npm run dev:full` so Vite and the `/api` handlers run side by side.
   Add `CAREERONESTOP_USERID` / `CAREERONESTOP_TOKEN` there too for the live
