...
export default function Scholarships({ user, storageAllowed }) {
  ...
  const [api, setApi] = useState({ state: 'idle', fetchedAt: null }) // idle | loading | ok | error
  const nationalAbort = useRef(null)
  const profileRef = useRef(profile)
  profileRef.current = profile
  const lastSaved = useRef(JSON.stringify(EMPTY_PROFILE))
  const uid = user?.uid
  const email = user?.email
  const canStore = Boolean(user) && storageAllowed === true // opted in on the account page
  ...
  // Debounced, abortable, and it never blocks the curated results: any failure
  // just leaves the curated set on screen.
  useEffect(() => {
    if (!query) {
      setNational([])
      setApi({ state: 'idle', fetchedAt: null })
      return
    }

    const timer = setTimeout(async () => {
      nationalAbort.current?.abort()
      const controller = new AbortController()
      nationalAbort.current = controller
      setApi({ state: 'loading', fetchedAt: null })
      try {
        const data = await searchNational(query, { signal: controller.signal })
        if (controller.signal.aborted) return
        setNational(data.scholarships)
        setApi({ state: 'ok', fetchedAt: data.fetched_at })
      } catch {
        if (controller.signal.aborted) return // superseded by a newer query
        setNational([])
        setApi({ state: 'error', fetchedAt: null })
      }
    }, 400)
  ...
  // Curated rows win a name clash, so the hand-verified local awards keep their
  // rich eligibility signals and the live rows only add what we don't have.
  const pool = useMemo(() => mergeByName(curated, national), [national])

  const fallbackMatches = useMemo(
    () => matchScholarships(pool, profile, { limit: 8 }),
    [pool, profile],
  )
  ...
  useEffect(() => {
    ...
    timer = setTimeout(async () => {
      try {
        const result = await advancedScholarshipMatches(pool, profile, { limit: 8 })
        if (!cancelled) {
          setAdvanced(result)
  ...
  // "already emailed" list has to be stored to avoid re-sending. Keyed on the
  // ids so a re-render with the same five matches doesn't re-hit Firestore.
  const topFiveRef = useRef(topFive)
  topFiveRef.current = topFive
  const topFiveKey = topFive.map((match) => match.scholarship.id).join(',')
  useEffect(() => {
    if (canStore && uid && email && topFiveKey) {
      alertNewGoodScholarships(topFiveRef.current, uid, email)
    }
  }, [topFiveKey, uid, email, canStore])
    ...
    <section className="wrap">
      ...
      <div className="sch-layout">
        ...
        <div className="sch-results">
          ...
          {hasProfile && (
            <div className="ranking-status" role="status">
              <span className={'ranking-dot ranking-dot--' + (api.state === 'ok' ? 'live' : 'local')} />
              {sourceLabel(api, curated.length, national.length)}
            </div>
          )}
...
function sourceLabel(api, curatedCount, nationalCount) {
  if (api.state === 'loading') return `Searching the national database, ${curatedCount} curated scholarships so far`
  if (api.state === 'ok') {
    return `${curatedCount} curated scholarships plus ${nationalCount} live national result${nationalCount === 1 ? '' : 's'}` +
      (api.fetchedAt ? `, as of ${api.fetchedAt}` : '')
  }
  if (api.state === 'error') return `National database unavailable, showing the ${curatedCount} curated scholarships`
  return `${curatedCount} curated scholarships. Add a major or interest to pull live national results`
}
